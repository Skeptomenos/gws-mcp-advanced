...
const defaultSpec = `gws-mcp-advanced==${pkg.version}`;
...
function startViaUvx(spec, args) {
  return {
    command: "uvx",
    args: ["--from", spec, "gws-mcp-advanced", ...args]
  };
}
...
function startViaUvToolRun(spec, args) {
  return {
    command: "uv",
    args: ["tool", "run", "--from", spec, "gws-mcp-advanced", ...args]
  };
}
...
if (!launch) {
  console.error(
    "[gws-mcp-advanced] Missing required runtime: install `uv` to use this launcher.\n" +
...
child.on("error", (error) => {
  console.error(`[gws-mcp-advanced] Failed to start process: ${error.message}`);
  process.exit(1);
});
