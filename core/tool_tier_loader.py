...
class ToolTierLoader:
    ...
    def _load_config(self) -> dict:
        ...
        try:
            ...
            raise ValueError(f"Invalid YAML in tool tiers configuration: {e}") from e
        except Exception as e:
            raise RuntimeError(f"Failed to load tool tiers configuration: {e}") from e
    ...
    def get_tools_for_tier(self, tier: TierLevel, services: list[str] | None = None) -> list[str]:
        """
    ...
    def get_tools_up_to_tier(self, tier: TierLevel, services: list[str] | None = None) -> list[str]:
        """
    ...
    def get_services_for_tools(self, tool_names: list[str]) -> set[str]:
        ...
        for service, service_config in config.items():
            for _tier_name, tier_tools in service_config.items():
...
def resolve_tools_from_tier(tier: TierLevel, services: list[str] | None = None) -> tuple[list[str], list[str]]:
    """
