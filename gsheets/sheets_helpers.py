...
def _parse_a1_part(part: str, pattern: re.Pattern[str] = A1_PART_REGEX) -> tuple[int | None, int | None]:
    """
...
def _parse_a1_range(range_name: str, sheets: list[dict]) -> dict:
    ...
    if sheet_name:
        ...
        if target_sheet is None:
            available_titles = [sheet.get("properties", {}).get("title", "Untitled") for sheet in sheets]
            available_list = ", ".join(available_titles) if available_titles else "none"
            raise UserInputError(f"Sheet '{sheet_name}' not found in spreadsheet. Available sheets: {available_list}.")
    else:
    ...
    for sheet in spreadsheet.get("sheets", []) or []:
        ...
        for grid in sheet.get("data", []) or []:
            ...
            for row_offset, row_data in enumerate(grid.get("rowData", []) or []):
                if not row_data:
                    continue
                for col_offset, cell_data in enumerate(row_data.get("values", []) or []):
                    if not cell_data:
                        continue
                    error_value = (cell_data.get("effectiveValue") or {}).get("errorValue") or None
                    if not error_value:
                        continue
                    errors.append(
                        {
...
async def _fetch_detailed_sheet_errors(service, spreadsheet_id: str, a1_range: str) -> list[dict[str, str | None]]:
...
def _format_sheet_error_section(*, errors: list[dict[str, str | None]], range_label: str, max_details: int = 25) -> str:
    """
    ...
    suffix = f"\n... and {len(errors) - max_details} more errors" if len(errors) > max_details else ""
...
def _summarize_conditional_rule(rule: dict, index: int, sheet_titles: dict[int, str]) -> str:
    """
...
async def _fetch_sheets_with_rules(service, spreadsheet_id: str) -> tuple[list[dict], dict[int, str]]:
    """
...
def _select_sheet(sheets: list[dict], sheet_name: str | None) -> dict:
    ...
    available_titles = [sheet.get("properties", {}).get("title", "Untitled") for sheet in sheets]
    raise UserInputError(f"Sheet '{sheet_name}' not found. Available sheets: {', '.join(available_titles)}.")
    ...
    if parsed:
        for idx, val in enumerate(parsed):
            if not isinstance(val, (str, int, float)):
                raise UserInputError(f"condition_values[{idx}] must be a string or number, got {type(val).__name__}.")
...
def _parse_gradient_points(
    ...
    for idx, point in enumerate(parsed):
        ...
        if not point_type or point_type.upper() not in GRADIENT_POINT_TYPES:
            raise UserInputError(f"gradient_points[{idx}].type must be one of {sorted(GRADIENT_POINT_TYPES)}.")
