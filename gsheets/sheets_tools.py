...
async def get_spreadsheet_info(
    ...
    for sheet in sheets:
        ...
        if rules:
            sheets_info.append(_format_conditional_rules_section(sheet_name, rules, sheet_titles, indent="    "))
...
async def read_sheet_values(
    ...
    if _values_contain_sheets_errors(values):
        ...
        try:
            errors = await _fetch_detailed_sheet_errors(service, spreadsheet_id, detailed_range)
            detailed_errors_section = _format_sheet_error_section(errors=errors, range_label=detailed_range)
...
async def modify_sheet_values(
    ...
    if clear_values:
        ...
        if updated_values and _values_contain_sheets_errors(updated_values):
            ...
            detailed_range = _a1_range_for_values(updated_range, updated_values) or updated_range
            try:
                errors = await _fetch_detailed_sheet_errors(service, spreadsheet_id, detailed_range)
                detailed_errors_section = _format_sheet_error_section(errors=errors, range_label=detailed_range)
            except Exception as exc:
...
async def format_sheet_range(
    ...
    if not any([background_color, text_color, number_format_type]):
        raise UserInputError("Provide at least one of background_color, text_color, or number_format_type.")
    ...
    if number_format_type:
        ...
        if normalized_type not in allowed_number_formats:
            raise UserInputError(f"number_format_type must be one of {sorted(allowed_number_formats)}.")
...
async def add_conditional_formatting(
    ...
    state_text = _format_conditional_rules_section(sheet_title, new_rules_state, sheet_titles, indent="")
...
async def delete_conditional_formatting(
    ...
    state_text = _format_conditional_rules_section(target_sheet_name, new_rules_state, sheet_titles, indent="")
